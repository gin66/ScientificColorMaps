...
for (map, cmPair) in maps {
    ...
    if let cm = cmPair.1 {
        ...
        swiftCode.append("   ]")
        swiftCode.append("   static let \(map) = ScientificColorMaps(raw: ScientificColorMaps.\(map)_raw, categories: \(map)_category_raw)")
    }
    else {
        swiftCode.append("   static let \(map) = ScientificColorMaps(raw: ScientificColorMaps.\(map)_raw)")
    }
    swiftCode.append("}")
